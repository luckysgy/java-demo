...
public class MediaTransferFlvByJavacv extends MediaTransfer implements Runnable {
    private static final Logger log = LoggerFactory.getLogger(MediaTransferFlvByJavacv.class);
    /**
     * 运行状态
     */
    ...
    public MediaTransferFlvByJavacv(MediaInfo mediaInfo) {
        this.mediaInfo = mediaInfo;
    }

    public MediaTransferFlvByJavacv(MediaInfo mediaInfo, boolean transferFlag) {
     ...
     * 创建转码推流录制器
     */
    protected boolean createRecorder() {
...
//		long processTime = 0;// 上一帧处理耗时，用于延迟时间补偿，处理耗时不算进累积延迟
        int frameCount = 1;
        while (running && grabberStatus && recorderStatus){
            try {
